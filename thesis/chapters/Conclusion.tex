...
The validated probes were further assembled with a graphene sheet into atomistic models of the
sensing interface, using a pristine layer built with GOPY and a placement set at the equilibrium
physisorption distance reported for nucleobases on graphene. All $30$ models are free of steric
clashes. Beyond demonstrating that the panel can be assembled automatically, these models exposed a
selection criterion that the sequence-level analysis cannot reach: the fraction of each probe lying
within the Debye length of the surface varies from $2$ to $14\%$ at physiological ionic strength
and from $22$ to $100\%$ in dilute buffer, and it is essentially uncorrelated with the 3D
confidence. Interface geometry is thus an independent axis of probe quality. On the two criteria
combined, \emph{p5972} (\emph{algD}) stands out, being both the highest-confidence structure and
