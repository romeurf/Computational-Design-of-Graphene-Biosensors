...
$N=100$ sequences requested per gene. It follows the flow of the method: sequence retrieval and
...
by the overall confidence. The best candidates, which are strong \emph{both} in sequence
quality and in 3D, are listed in Table~\ref{tab:boltz}. Notably, they belong to the two
data-limited but highly conserved genes \emph{algD} and \emph{frdB}: their sequences are
strongly conserved (\acrshort{ppi} up to $1.00$) and essentially free of self-structure
(No-fold up to $100$), and this translates into the highest predicted-structure confidence and
\acrshort{plddt}. This coherence between the sequence-level signals and the independent 3D
prediction is a reassuring internal validation of the ranking.
...
\hline
p5972 & \emph{algD} & 0.93 & 98  & 0.731 & 0.901 \\
p7716 & \emph{frdB} & 0.99 & 100 & 0.723 & 0.822 \\
p5938 & \emph{algD} & 0.90 & 96  & 0.721 & 0.887 \\
p8448 & \emph{frdB} & 1.00 & 98  & 0.716 & 0.845 \\
\hline
\end{tabular}
\end{center}
...
% Suggested figure: copy the analysis output into thesis/images/ and uncomment.
% \begin{figure}[H]\centering
%   \includegraphics[width=0.7\textwidth]{images/boltz2_confidence.png}
%   \caption{Distribution of Boltz-2 confidence over the 30 validated probes.}
%   \label{fig:boltz}
% \end{figure}
...
allowance of more sequences. The cosine and Jaccard distances agree in their ordering of the
genes, giving a robust, alignment-independent reading of diversity.
...
The rarefaction analysis showed that both the $k$-mer diversity and the $k$-mer richness
saturate at around $N \approx 50$--$75$ sequences for the most demanding gene (\emph{nuc}), and
much earlier for the others. Requesting $N \approx 100$ sequences per gene therefore captures
essentially all of the available diversity with a comfortable margin, while additional
sequences add little new information. The true limiting factor is not $N$ but the small number
of records available in \acrshort{ncbi} for the data-limited genes.

% \begin{figure}[H]\centering
%   \includegraphics[width=0.7\textwidth]{images/rarefaction.png}
%   \caption{Rarefaction curves of $k$-mer richness per gene, saturating by $N\approx 50$--$75$.}
...
directly. On average, the reference probes that pass the screen sit slightly below the designed
ones on the sequence-quality signals (for instance in mean \acrshort{tm} and No-fold score),
which is consistent with their being an older and more heterogeneous panel. Because both sets
carry a source label and share every column, they can also be submitted together to Boltz-2,
enabling a like-for-like comparison in three dimensions as well as in sequence space.
...
in 3D and provides labelled, biophysically annotated examples for data-driven modelling of
probe quality, which is a natural continuation of this work.
